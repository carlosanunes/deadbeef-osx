...
cocoaui_message (uint32_t id, uintptr_t ctx, uint32_t p1, uint32_t p2) {
    ...
    DBAppDelegate * delegate = [NSApp delegate];
    if (delegate == nil)
        return 0;

    
    [delegate performSelectorOnMainThread: @selector(handleMessage:) withObject:[[NSNumber alloc] initWithInt: id] waitUntilDone: NO];
